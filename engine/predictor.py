...
Two weight configs are used (see GROUP_WEIGHTS / KNOCKOUT_WEIGHTS):
- Group stage: 25% FIFA rank, 25% form, 25% goal diff, 25% news readiness.
- Knockout stage: 50% FIFA rank, ~16.7% form, ~16.7% goal diff, ~16.7% news.
...
Score grid: independent Poisson with a Dixon-Coles low-score correction.
Two-stage argmax: fix outcome (win/draw/loss) by probability mass, then
pick the most probable scoreline within that outcome region.
"""

import math
from bisect import bisect_right

GROUP_WEIGHTS    = {"rank": 0.25, "form": 0.25, "goals": 0.25, "news": 0.25}
KNOCKOUT_WEIGHTS = {"rank": 0.50, "form": 1.0 / 6, "goals": 1.0 / 6, "news": 1.0 / 6}
...
class TeamModel:
    ...
    def __init__(self, code, fifa_points, recent_matches, min_pts, max_pts,
        ...
        w = weights if weights is not None else GROUP_WEIGHTS
        self.power = (w["rank"] * self.rank_score
                      + w["form"] * form
                      + w["goals"] * self.goal_score
                      + w["news"] * news_norm)
